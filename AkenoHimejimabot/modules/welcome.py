...
from AkenoHimejimabot import (DEV_USERS, LOGGER, OWNER_ID, DRAGONS, DEMONS, TIGERS,
                          WOLVES, sw, dispatcher, JOIN_LOGGER)
from AkenoHimejimabot.modules.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from AkenoHimejimabot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from AkenoHimejimabot.modules.helper_funcs.msg_types import get_welcome_type
from AkenoHimejimabot.modules.helper_funcs.string_handling import (
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "President! Where is our Issei-kun?",
                    reply_to_message_id=reply)
            ...
            # Welcome Devs
                ...
                update.effective_message.reply_text(
                    "Rias-sama was expecting you to come",
                    reply_to_message_id=reply,
                )
                continue
    ...
    if not member_status:
        ...
        try:
            bot.edit_message_text(
                "My my.. Why you are in so rush...Just be gentle.",
                chat_id=chat_id,
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "Sayounara Rias-sama....Tell Issei that I wanted to heal him <3", reply_to_message_id=reply)
                return

            # Give the devs a special goodbye
            elif left_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "President Rias will miss you...",
                    reply_to_message_id=reply,
                )
                return
